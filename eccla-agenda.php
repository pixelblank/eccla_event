...
function eccla_agenda_add_metaboxes() {
    add_meta_box('eccla_event_details', 'Détails de l\'événement', 'eccla_agenda_render_metabox', 'eccla_event', 'side', 'high');
}
...
add_action('save_post', function($post_id) {
    ...
    $fields = ['event_date', 'event_end_date', 'event_start_time', 'event_end_time', 'event_pdf'];
...
add_shortcode('eccla_agenda', function() {
    ...
    while ($query->have_posts()) {
        ...
        if ($date) {
            ...
            // Si pas de date de fin, on prend la date de début
            $final_end_date = $end_date ?: $date;
            $end = $final_end_date . ($end_time ? 'T' . $end_time : '');
